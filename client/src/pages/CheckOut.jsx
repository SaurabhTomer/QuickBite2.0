...
//we call this outside checkout function due to rendering error
//function which  is called to recenter map (means when we move dragger then map shift accordig to dragger)
...
function CheckOut() {
    ...
    // funtion which is called to get lat and long of loaction when we drag marker on  map
    const onDragEnd = (e) => {
        //get lat and lng from event
        const { lat, lng } = e.target._latlng

        dispatch(setLocation({ lat, lon: lng }))    // set lat and long on mapslice
        getAddressByLatLng(lat, lng)        //calls function which give location make from lat and long 
    }


    // funtion to get cureent location  when we click on current loaction icon  by browser location to get current location
    ...
    //funtion to get address from lat and long on map
    const getAddressByLatLng = async (lat, lng) => {
        try {
            // reverse geocoding api to get address from lat and lng
    ...
    //funtion calls when we search address on input label and we get lat and lang on map
    const getLatLngByAddress = async () => {
        try {
            // we use forward geoocoding api to get lat , lng of current address
            ...
            <div className='w-full max-w-225 bg-white rounded-2xl shadow-xl p-6 space-y-6'>
                ...
                <section>
                    ...
                    <div className='flex gap-2 mb-3'>
                        <input type="text" className='flex-1 border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#ff4d2d]'
                            placeholder='Enter Your Delivery Address..' value={addressInput}
                            onChange={(e) => setAddressInput(e.target.value)} />

                        <button className='bg-[#ff4d2d] hover:bg-[#e64526] text-white px-3 py-2 rounded-lg flex items-center justify-center'
                            onClick={getLatLngByAddress}><IoSearchOutline size={17} /></button>

                        {/* button to get current location */}
                        <button className='bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg flex items-center justify-center'
                            onClick={getCurrentLocation}><TbCurrentLocation size={17} /></button>
                    </div>
                    ...
                    <div className='rounded-xl border overflow-hidden'>
                        ...
                        <div className='h-64 w-full flex items-center justify-center'>
                            <MapContainer
                                ...
                                center={[location?.lat, location?.lon]}         // indicates towards loacation on map
                                zoom={16}
                            >
                                <TileLayer
                                ...
                                {/* marker which shows location */}
                                <Marker position={[location?.lat, location?.lon]}
                                    draggable
                                    eventHandlers={{ dragend: onDragEnd }} />   // this is used to get location
